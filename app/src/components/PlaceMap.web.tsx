...
import type { PlaceMapRef } from "./PlaceMap";
    ...
    useEffect(() => {
      ...
      visiblePlaces.forEach((place) => {
        ...
        const marker = L.marker([place.coordinates[1], place.coordinates[0]], {
          icon: markerIcon(categoryColors[category] ?? "#2f6861"),
        });
        ...
        marker.bindPopup(
          `<div class="vistytis-place-popup"><strong style="display:block">${title}</strong><span style="display:block;margin-top:4px;color:#2f6861;font-size:11px;font-weight:700">${categoryName}</span><button type="button" style="margin-top:8px">${openLabel}</button></div>`
        );
